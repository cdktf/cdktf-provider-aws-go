...
func (j *jsiiProxy_DataAwsGlueScriptDagNodeList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*DataAwsGlueScriptDagNode:
		val := val.(*[]*DataAwsGlueScriptDagNode)
	...
	case []*DataAwsGlueScriptDagNode:
		val_ := val.([]*DataAwsGlueScriptDagNode)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*DataAwsGlueScriptDagNode; received %#v (a %T)", val, val)
		}
	}
