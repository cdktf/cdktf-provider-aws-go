...
import (
	...
	"github.com/hashicorp/cdktf-provider-aws-go/aws/v10/glaciervault/internal"
...
func NewGlacierVault(scope constructs.Construct, id *string, config *GlacierVaultConfig) GlacierVault {
	...
	if err := validateNewGlacierVaultParameters(scope, id, config); err != nil {
		panic(err)
	}
...
func GlacierVault_IsConstruct(x interface{}) *bool {
	...
	if err := validateGlacierVault_IsConstructParameters(x); err != nil {
		panic(err)
	}
	var returns *bool
...
func (g *jsiiProxy_GlacierVault) PutNotification(value *GlacierVaultNotification) {
	if err := g.validatePutNotificationParameters(value); err != nil {
		panic(err)
	}
	_jsii_.InvokeVoid(
		g,
