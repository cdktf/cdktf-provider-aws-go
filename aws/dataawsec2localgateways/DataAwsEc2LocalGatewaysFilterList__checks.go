...
func (d *jsiiProxy_DataAwsEc2LocalGatewaysFilterList) validateResolveParameters(context cdktf.IResolveContext) error {
	if context == nil {
...
func (j *jsiiProxy_DataAwsEc2LocalGatewaysFilterList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*DataAwsEc2LocalGatewaysFilter:
		val := val.(*[]*DataAwsEc2LocalGatewaysFilter)
	...
	case []*DataAwsEc2LocalGatewaysFilter:
		val_ := val.([]*DataAwsEc2LocalGatewaysFilter)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*DataAwsEc2LocalGatewaysFilter; received %#v (a %T)", val, val)
		}
	}
