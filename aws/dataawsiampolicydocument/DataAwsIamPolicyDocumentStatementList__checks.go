...
func (d *jsiiProxy_DataAwsIamPolicyDocumentStatementList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_DataAwsIamPolicyDocumentStatementList) validateSetInternalValueParameters(val interface{}) error {
	...
	case []*DataAwsIamPolicyDocumentStatement:
		val_ := val.([]*DataAwsIamPolicyDocumentStatement)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
