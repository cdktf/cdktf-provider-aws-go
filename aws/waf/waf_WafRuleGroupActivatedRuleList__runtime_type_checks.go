...
func (j *jsiiProxy_WafRuleGroupActivatedRuleList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*WafRuleGroupActivatedRule:
		val := val.(*[]*WafRuleGroupActivatedRule)
	...
	case []*WafRuleGroupActivatedRule:
		val_ := val.([]*WafRuleGroupActivatedRule)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*WafRuleGroupActivatedRule; received %#v (a %T)", val, val)
		}
	}
