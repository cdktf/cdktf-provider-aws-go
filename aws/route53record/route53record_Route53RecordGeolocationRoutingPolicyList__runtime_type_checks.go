...
func (j *jsiiProxy_Route53RecordGeolocationRoutingPolicyList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*Route53RecordGeolocationRoutingPolicy:
		val := val.(*[]*Route53RecordGeolocationRoutingPolicy)
	...
	case []*Route53RecordGeolocationRoutingPolicy:
		val_ := val.([]*Route53RecordGeolocationRoutingPolicy)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*Route53RecordGeolocationRoutingPolicy; received %#v (a %T)", val, val)
		}
	}
